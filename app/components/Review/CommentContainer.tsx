'use client'
import { useState } from "react";
import CommentForm from "../Comment/CommentForm";
import CommentCard from "../Comment/CommentCard";
import React from "react";
import LikeCount from "./LikeCount";
import { Comment } from "./ReviewInterfaces";


interface CommentContainerProps {
    submitCommentForm: (event: React.FormEvent<HTMLFormElement> | any) => void; // Adjust according to the actual parameter
    comments: Comment[];   
    likeState: boolean; // Optional if not always required
    likeCount: number; // Optional if not always required
    createLikeEntry?: any; // Optional, adjust `likeData` type as needed
    isUser?: boolean; // Optional if not always required
    commentCount?: number; // Optional if not always required
}

const CommentContainer: React.FC<CommentContainerProps> = ({
    submitCommentForm,    
    comments,
    likeState = null,
    likeCount = 0,
    createLikeEntry,
    isUser = false,
    commentCount = 0,
}) => {

    const [showComments, setShowComments] = useState(false)

    const handleCommentClick = () => {
        setShowComments(!showComments)
    }

    return (
        <>
            <div className="flex flex-row items-center gap-2 ml-auto mb-4">
                {likeState !== null && likeCount !== null && <LikeCount createLikeEntry={createLikeEntry} isUser={isUser} likeState={likeState} likes={likeCount} />}
                <svg xmlns="http://www.w3.org/2000/svg" onClick={handleCommentClick} fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" className="w-6 h-6 cursor-pointer ">
                    <path stroke-linecap="round" stroke-linejoin="round" d="M8.625 12a.375.375 0 1 1-.75 0 .375.375 0 0 1 .75 0Zm0 0H8.25m4.125 0a.375.375 0 1 1-.75 0 .375.375 0 0 1 .75 0Zm0 0H12m4.125 0a.375.375 0 1 1-.75 0 .375.375 0 0 1 .75 0Zm0 0h-.375M21 12c0 4.556-4.03 8.25-9 8.25a9.764 9.764 0 0 1-2.555-.337A5.972 5.972 0 0 1 5.41 20.97a5.969 5.969 0 0 1-.474-.065 4.48 4.48 0 0 0 .978-2.025c.09-.457-.133-.901-.467-1.226C3.93 16.178 3 14.189 3 12c0-4.556 4.03-8.25 9-8.25s9 3.694 9 8.25Z" />
                </svg>
                <p className="-ml-1 text-sm text-gray-500">{commentCount}</p>
            </div>
            {showComments && <>
                <CommentForm submitCommentForm={submitCommentForm} />
                {comments.map((comment, index) => (
                    <>
                        <CommentCard comment={comment} />
                    </>
                ))}
            </>}
        </>
    )
}

export default CommentContainer;